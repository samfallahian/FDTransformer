#!/usr/bin/env python3
"""
validation_generic_overlapping_encoding_detail.py - Benchmark overlapping encodings.

Creates a dataframe with coordinates and their neighbors for model comparison.
Uses embedded model definition from checkpoint file.
"""
...
from encoder.permutations.model_09_residual_ae import ResidualAE  # noqa: E402
...
# Model checkpoint path - uses embedded model definition
MODEL_CHECKPOINT = "/Users/kkreth/PycharmProjects/cgan/encoder/saved_models/Model_09_Residual_AE_epoch_500.pt"
...
def load_model_from_checkpoint(checkpoint_path):
    """
    Load model from checkpoint (instantiates model and loads state dict).
    ...
    # Instantiate model
    model = ResidualAE(dropout_rate=0.2)
    ...
    if isinstance(checkpoint, dict):
        if 'model_state_dict' in checkpoint:
            ...
            logger.error(f"Checkpoint is dict with keys: {list(checkpoint.keys())}")
            raise ValueError(f"Cannot find model or model_state_dict in checkpoint")
    else:
