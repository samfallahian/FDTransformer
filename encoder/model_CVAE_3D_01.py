...
class UnFlatten(nn.Module):
    ...
    def forward(self, input, size=128):  # Changed from 512
        return input.view(input.size(0), size, 1, 1, 1)  # Changed from 3,3,3
...
class CVAE_3D(nn.Module):
    def __init__(self, image_channels=3, h_dim=128, z_dim=32):
        ...
        print()
        print("[INFO] instantiating pytorch model: 3D CVAE (Modified for 8x8x8 output)")
        ...
        self.encoder = nn.Sequential(
            ...
            # Final layer
            nn.Conv3d(in_channels=32, out_channels=128, kernel_size=2, stride=1, padding=0),
        ...
        # Modified decoder for precise 8x8x8 output
        self.decoder = nn.Sequential(
            UnFlatten(),  # Outputs [B, 128, 1, 1, 1]
            nn.BatchNorm3d(num_features=128),
            nn.ReLU(),
            # First transpose conv: 1→4
            nn.ConvTranspose3d(in_channels=128, out_channels=64, 
                              kernel_size=4, stride=1, padding=0),  # Output: [B, 64, 4, 4, 4]
            nn.BatchNorm3d(num_features=64),
            nn.ReLU(),
            # Second transpose conv: 4→8
            nn.ConvTranspose3d(in_channels=64, out_channels=32, 
                              kernel_size=5, stride=1, padding=0),  # Output: [B, 32, 8, 8, 8]
            nn.BatchNorm3d(num_features=32),
            nn.ReLU(),
            # Final transpose conv for channel reduction, maintaining 8x8x8
            nn.ConvTranspose3d(in_channels=32, out_channels=image_channels, 
                              kernel_size=3, stride=1, padding=1),  # Output: [B, 3, 8, 8, 8]
