...
class CustomDataset(Dataset):
    ...
    def __len__(self):
        return len(self.df) * (self.df.columns.size - self.source_len)

    def __getitem__(self, idx):
        row_idx = idx // (self.df.columns.size - self.source_len)
        col_idx = idx % (self.df.columns.size - self.source_len)

        src_values = [self.df.iloc[row_idx, col_idx + i] for i in range(self.source_len)]
        x = torch.tensor(np.array(src_values), dtype=torch.float32)

        y_values = self.df.iloc[row_idx, col_idx + self.source_len]
