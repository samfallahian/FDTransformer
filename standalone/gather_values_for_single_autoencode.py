...
randomSingleton = df.sample()
randomTimeHappenedToB = randomSingleton.time
aDifficultFloatToGather = randomTimeHappenedToB.iloc[0]
...
if os.path.exists(pickle_file):
    ...
    df_subset = df[df['time'] == aDifficultFloatToGather]
    # Save the DataFrame subset to a pickle file
...
#So, now with this information, can we use this data?
...
try:
    ...
    print("Y is within bounds.")
except ValueError as e:
    print(str(e))

try:
    check_float_bounds(arandomz, z_bound_lower, z_bound_upper)
    print("Z is within bounds.")
except ValueError as e:
    print(str(e))

#Here we will store the 125 rows
'''
df_rawTarget = df_subset[df_subset['x'] == arandomx]
df_rawTarget = df_rawTarget[df_rawTarget['y'] == arandomy]
df_rawTarget = df_rawTarget[df_rawTarget['z'] == arandomz]
'''
