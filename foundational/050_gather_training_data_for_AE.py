...
def process_file(filename, META_DATA_FILE_PATH):
    ...
    with h5py.File(output_file_path, 'w') as hf:
        for i, tensor in enumerate(velocity_tensors):
            # Convert coordinates to a string where each coordinate is separated by an underscore
            coordinates_str = '_'.join([f'{x},{y},{z}' for x, y, z in coordinates])

            # Use this string as a part of the dataset name
            hf.create_dataset(f'velocity_tensor_{coordinates_str}', data=tensor.numpy())
